...
struct MacRootView: View {
    ...
    /// The board. One definition, because it is drawn in two places — with the chat column and
    /// without it.
    ///
    /// It used to carry the list's floating quick-add pinned under the whole board (e466eab8),
    /// on top of the "+ Add task" at the foot of every column — two boxes for one job, and they
    /// disagreed about what a new task would be. Each column now draws the list's own bar
    /// (`MacQuickAddBar`), so the central one is gone (AITD-431).
    ...
    ///
    /// The bar itself is `MacQuickAddBar`, shared with every board column (AITD-431). What stays
    /// here is what only the list column knows: where the task goes, when the caret belongs in
    /// the field, and that ⊕ opens the new task's details.
    private var quickAddBar: some View {
        MacQuickAddBar(selectedListId: selectedListId,
                       selectionIsVirtual: selectionIsVirtual,
                       list: currentRealList,
                       takesFocus: MacAddTaskBar.shouldTakeFocus(
                           isVisible: MacAddTaskBar.isVisible(isVirtualSelection: selectionIsVirtual,
                                                              hasSelection: selectedListId != nil,
                                                              isMyTasks: selectedListId == Self.myTasksId),
                           isSearchActive: !taskSearchQuery.isEmpty),
                       focusKey: "\(selectedListId ?? "")|\(taskSearchQuery.isEmpty)",
                       onOpen: { selectedTaskIds = [$0.id] })
    }
